...
import { useAddNewBudgetItem } from "../../hooks/useNewBudgetItem";
...
export const AddNewBudgetItem: React.FC<AddNewBudgetItemProps> = (props) => {
  ...
  const { fields, screen, next, save, back, update } = useAddNewBudgetItem();

  const screens = [
    { title: "Add New Budget Item", action: { label: "Next", cancel: "Cancel", actionFn: () => { next() } } },
    { title: "Add New Budget Item (cont'd)", action: { label: "Finish", cancel: "Back", actionFn: () => { save() }, cancelFn: () => back() } }
  ];

  const currentScreen = screens[screen];

  return (
    <Modal title={currentScreen.title} isOpen={isOpen} setIsOpen={setIsOpen} action={currentScreen.action}>
      {screen === 0 && <GeneralScreen
        // name={fields.name}
...
const FrequencyScreen: React.FC<FrequencyScreenProps> = (props) => {
  ...
  const isWeekly = frequency === BudgetLineItemFrequency.Weekly || frequency === BudgetLineItemFrequency.Biweekly;
            ...
            {isWeekly &&
              <div className="col-6">
                ...
                <Select
                  ...
                  options={
                    Object.keys(WeeklyBudgetLineItemDays).map(day => ({ label: day, value: day }))
                  }
