...
export const login = async (user: string, password: string): Promise<x25Result<LoginResponse>> => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        const err = await response.json() as StrapiErrorResponse

        // Assuming the server sends an error object with a 'message' property
        if (err.error.message) {
          message = err.error.message;
        }

      } catch {
        x25log.d("[login][strapi.ts]: Error parsing error message from strapi.");
        // If JSON parsing fails, try to get plain text or a generic error
        // return response.text().then(text => {
        //   throw new Error(text || 'Server error, could not parse message');
        // });
      };

      return { status: "fail", error: `${message} (${response.status})` }
    }
