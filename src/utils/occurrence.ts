...
// export function calculateOccurrences(_item: x25Item|x25Item[], start: Dayjs, end: Dayjs): Occurrence[] { 
  ...
  items.forEach( item => {
    ...
    // if (item.frequency === "Once") {
      ...
      item.dates.split(",").forEach((_, i) => {
        ...
        x25log.d("[calculateOccurrences][occurrence.ts]: %s occurrs on %s.", item.name ?? "--", firstOccurrence.format("YYYY-MM-DD"))
  
        if (firstOccurrence.isAfter(end, 'date')) {
          x25log.d("[calculateOccurrences][occurrence.ts]: %s does not occur between %s and %s.", item.name ?? "--", start.format("YYYY-MM-DD"), end.format("YYYY-MM-DD"));
          return;
        }
  ...
  x25log.d("[calculateOccurrences][occurrence.ts]: Returning %d occurrences.", output.length);
  return output;
}


// type CalculateBalanceAt = "StartofDay" | "EndofDay";
// export const calculateBalance = (startDate: Dayjs, startingAmount: number, items: Schema<"item">[], endDate?: Dayjs, at: CalculateBalanceAt = "StartofDay"): number | string => {
...
//         // Check if verified.
//         const vAmount = getVerificationOn(date, item);
