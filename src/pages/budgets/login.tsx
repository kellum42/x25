...
const UsernameLogin: React.FC<{redirect: string}> = ({ redirect }) => {
  ...
  return (
    <div className="">
      {!isActive &&
        <span onClick={() => { setIsActive(true) }} className="text-gray-400 text-decoration-underline">Username/password</span>
      }
      {isActive &&
        <div className="text-start">
          <div className="fv-row mb-10 fv-plugins-icon-container">
            <label className="form-label fs-6 fw-bold text-dark">Email</label>
            <input className="form-control form-control-lg form-control-solid" onChange={onUsernameChange} type="text" name="email" autoComplete="off" />
          ...
          <div className="fv-row mb-10 fv-plugins-icon-container">
            ...
            <input className="form-control form-control-lg form-control-solid" onChange={onPasswordChange} type="password" name="password" autoComplete="off" />
            <div className="fv-plugins-message-container invalid-feedback"></div>
          </div>
          <div className="text-center">
            <button type="button" onClick={() => { credentialedLogin() }} className="btn btn-lg btn-secondary w-100 mb-5" data-kt-indicator={isLoading ? "on" : "off"}>
              <span className="indicator-label">Login</span>
              <span className="indicator-progress">Please wait...
                <span className="spinner-border spinner-border-sm align-middle ms-2"></span></span>
            </button>
          </div>
          { error && <p className="badge-light-danger p-2">{error}</p> }
          <div>
            <span onClick={() => { setIsActive(false) }} className="text-gray-400 text-decoration-underline">Close</span>
          </div>
        </div>
      }
    </div>
...
const Login: React.FC<RouteComponentProps> = (props) => {
  ...
  const onDemoLogin = async () => {
    ...
    if (hasLoggedIn.status === "success" ) {
      ...
      x25log.d("[onDemoLogin][login.tsx]: Login failed. %s", hasLoggedIn.error);
      // navigate("/budgets")
    }
  }
          ...
          <div className="d-flex flex-center flex-column flex-column-fluid">
            <div className="w-lg-500px p-10 p-lg-15 mx-auto w-100">
              <form className="form w-100 fv-plugins-bootstrap5 fv-plugins-framework" id="kt_sign_in_form" data-kt-redirect-url="../dist/index.html" action="#">
                ...
                <div className="text-center">
                  <button type="button" onClick={() => { onDemoLogin() }} className="btn btn-lg btn-primary w-100" data-kt-indicator={loading ? "on" : "off"}>
